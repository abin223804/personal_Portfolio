...
export const ProjectGrid: React.FC<{ limit?: number }> = ({ limit }) => {
  ...
  return (
    <section className="py-20 bg-obsidian-bg relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-mono transition-all ${
                    selectedCategory === cat
                      ...
                      : "bg-obsidian-surface text-titanium hover:text-ivory border border-obsidian-border"
                  }`}
                >
                  {cat}
                </button>
        ...
        {limit && PROJECTS.length > limit && (
          <div className="mt-12 text-center">
            <Link
              href="/projects"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-obsidian-surface hover:bg-obsidian-card border border-obsidian-border hover:border-copper text-ivory text-xs font-mono font-bold transition-all shadow-lg"
            >
