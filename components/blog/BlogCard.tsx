        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:items-center justify-between relative z-10">
          <div className="space-y-4 max-w-3xl">
            <div className="flex flex-wrap items-center gap-3">
              <span className="px-3 py-1 rounded-full bg-copper/20 border border-copper/50 text-copper-light text-xs font-mono font-bold flex items-center gap-1.5">
                <BookOpen className="w-3.5 h-3.5" />
                Featured Engineering Article
              </span>
              <span className="text-xs text-titanium font-mono flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 text-copper-light" />
                {post.date}
              </span>
              <span className="text-xs text-titanium font-mono flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 text-copper-light" />
                {post.readTime}
              </span>
            </div>

            <h2 className="text-2xl sm:text-4xl font-extrabold text-ivory tracking-tight group-hover:text-copper-light transition-colors leading-tight">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2">
              {post.tags.map((tag) => (
                ...
                >
                  <Tag className="w-3 h-3 text-copper-light" />
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-4 lg:flex-col lg:items-end justify-between pt-4 lg:pt-0 border-t lg:border-t-0 border-obsidian-border">
            <Link
              href={`/blog/${post.slug}`}
              className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-copper hover:bg-copper-light text-obsidian-bg text-xs font-extrabold shadow-lg shadow-copper/20 transition-all transform group-hover:translate-x-1"
            >
    ...
    <article className="group bg-obsidian-surface/60 border border-obsidian-border hover:border-copper/50 rounded-xl p-6 transition-all duration-300 flex flex-col justify-between hover:shadow-xl relative overflow-hidden">
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-titanium font-mono">
          <span className="px-2.5 py-0.5 rounded-full bg-obsidian-card border border-obsidian-border text-copper-light text-[11px] font-bold">
            {post.category}
          </span>
